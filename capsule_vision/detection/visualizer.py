...
def annotate(frame: np.ndarray, result: OrientationResult) -> np.ndarray:
    ...
    if not result.detected:
        # Always draw the YOLO box if the AI did see something, even if
        # the OpenCV secondary analysis failed.
        if result.bbox is not None:
            x1, y1, x2, y2 = result.bbox
            cv2.rectangle(out, (x1, y1), (x2, y2), _PURPLE, 2)
            label = f"AI conf={result.ai_conf:.2f}  (OpenCV analysis failed)"
            cv2.putText(out, label, (x1, max(y1 - 8, 14)),
                        _FONT, 0.5, _PURPLE, 1, cv2.LINE_AA)
            cv2.putText(out, "Capsule detected — angle calc failed", (20, 40),
                        _FONT, 0.7, _YELLOW, 2, cv2.LINE_AA)
        else:
            cv2.putText(out, "No capsule detected  (YOLO: 0 boxes)", (20, 40),
    ...
    if result.ellipse is not None:
        (cx, cy), axes, angle = result.ellipse
        # cv2.ellipse with RotatedRect form requires integer center
        safe_ellipse = ((int(cx), int(cy)), (int(axes[0]), int(axes[1])), float(angle))
        cv2.ellipse(out, safe_ellipse, _CYAN, 2)

    # ── 4. Major axis line + seam marker ──────────────────────────────────
    if result.ellipse is not None:
        (cx, cy), (ax0, ax1), angle = result.ellipse
        ...
        # Seam tick (perpendicular to major axis)
        seam_offset = result.seam_ratio - 0.5
